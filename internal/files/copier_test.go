...
func TestCopyWithAddPerm_Success_SingleFile(t *testing.T) {
	// Create temporary source directory
	srcDir := t.TempDir()
	dstDir := t.TempDir()
	...
	copier := NewCopier()
	err = copier.CopyWithAddPerm(srcDir, dstDir, 0666)
	assert.NoError(t, err, "CopyWithAddPerm failed")
	...
	assert.Equal(t, os.FileMode(0666), fileInfo.Mode().Perm(), "file permissions should be 0666")
}

func TestCopyWithAddPerm_Success_NestedDirectories(t *testing.T) {
	srcDir := t.TempDir()
	dstDir := t.TempDir()
	...
	err = os.WriteFile(rootFile, []byte("root content"), 0700)
	assert.NoError(t, err, "Failed to create root file")
	err = os.WriteFile(nestedFile, []byte("nested content"), 0644)
	assert.NoError(t, err, "Failed to create nested file")
	...
	// Verify root file permissions
	fileInfo, err := os.Stat(dstRootFile)
	assert.NoError(t, err, "Failed to get root file info")
	assert.Equal(t, os.FileMode(0766), fileInfo.Mode().Perm(), "root file permissions should be 0666")
	...
	fileInfo, err = os.Stat(dstNestedFile)
	assert.NoError(t, err, "Failed to get nested file info")
	assert.Equal(t, os.FileMode(0666), fileInfo.Mode().Perm(), "nested file permissions should be 0666")
}

func TestCopyWithAddPerm_Error_SourceNotExists(t *testing.T) {
	dstDir := t.TempDir()
	nonExistentSrc := filepath.Join(dstDir, "nonexistent")

	copier := NewCopier()
	err := copier.CopyWithAddPerm(nonExistentSrc, dstDir, 0666)
	assert.Error(t, err, "should return error when non existent source")
}

func TestCopyWithAddPerm_Error_InvalidDestination(t *testing.T) {
	srcDir := t.TempDir()
	...
	copier := NewCopier()
	err = copier.CopyWithAddPerm(srcDir, invalidDst, 0666)
	assert.Error(t, err, "should return error when non existent destination")
}

func TestCopyWithAddPerm_Success_EmptyDirectory(t *testing.T) {
	srcDir := t.TempDir()
	dstDir := t.TempDir()
	...
	copier := NewCopier()
	err := copier.CopyWithAddPerm(srcDir, dstDir, 0666)
	assert.NoError(t, err, "CopyWithAddPerm of empty directory shouldn't failed")
