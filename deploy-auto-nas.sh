...
mkdir -p "$TARGET_DIR"
echo "📂  Copying source files from $SCRIPT_DIR to $TARGET_DIR"
...
mkdir -p "$TARGET_DIR/services/homepage/config"

# Prepare YAML file
yaml_file="$TARGET_DIR/services/homepage/config/services.yaml"
...
if [[ ${#SERVICES_LIST[@]} -eq 0 ]]; then
  ...
  for service in "${SERVICES_LIST[@]}"; do
    ...
    env_file_path="$TARGET_DIR/services/$service/.env"
    ...
    echo "WORK_DIR=$TARGET_DIR" >> "$env_file_path"
...
EOF
    ...
    cd "$TARGET_DIR/services/$service"
    docker compose up -d 
  done
fi
